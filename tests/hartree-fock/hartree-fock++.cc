...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        const auto basis_name = (argc > 2) ? argv[2] : "cc-pvdz";
        ...
        auto V =
            compute_1body_ints(obs, libint2::OneBodyEngine::nuclear, atoms);
        Matrix H = T + V;
        T.resize(0, 0);
        V.resize(0, 0);
        ...
        {  // use SOAD as the guess density
            ...
            auto D_minbs =
                compute_soad(atoms);  // compute guess in minimal basis
            BasisSet minbs("STO-3G", atoms);
            if (minbs == obs)
                D = D_minbs;
            else {  // if basis != minimal basis, map non-representable SOAD
                    // guess into the AO basis
                ...
                const std::chrono::duration<double> time_elapsed =
                    tstop - tstart;
                std::cout << "done (" << time_elapsed.count() << " s)"
                          << std::endl;
            }
        }
        ...
        do {
            ...
            // DIIS extrapolate F
            Matrix F_diis = F;  // extrapolated F cannot be used in incremental
                                // Fock build; only used to produce the density
        ...
        } while (((ediff_rel > conv) || (rms_error > conv)) &&
                 (iter < maxiter));

        printf("** Hartree-Fock energy = %20.12f\n", ehf + enuc);

        auto d_file_name = "D-" + std::string(basis_name) + ".binary";
        std::ofstream dfile(d_file_name.c_str());
        if (dfile.good()) {
            boost::archive::binary_oarchive out_archive(dfile,
                                                        std::ios::binary);
            out_archive << D;
        }
        dfile.flush();
...
std::vector<Atom> read_geometry(const std::string& filename) {
    ...
    // string that can be
    // broadcast to everyone, then converted to an std::istringstream object
    // that can be used just like std::ifstream
...
Matrix compute_1body_ints(const BasisSet& obs,
    ...
    engines[0] =
        libint2::OneBodyEngine(obtype, obs.max_nprim(), obs.max_l(), 0);
...
Matrix compute_schwartz_ints(const BasisSet& obs) {
    ...
    engines[0].set_precision(0.);  // !!! very important: cannot screen
                                   // primitives in Schwartz computation !!!
    for (size_t i = 1; i != nthreads; ++i) {
        engines[i] = engines[0];
    }
...
Matrix compute_2body_fock(const BasisSet& obs, const Matrix& D,
    ...
    const auto do_schwartz_screen =
        Schwartz.cols() != 0 && Schwartz.rows() != 0;
    ...
    std::vector<coulomb_engine_type> engines(nthreads);
    engines[0] = coulomb_engine_type(obs.max_nprim(), obs.max_l(), 0);
    engines[0].set_precision(std::min(
        precision,
        std::numeric_limits<double>::epsilon()));  // shellset-dependent
                                                   // precision control will
                                                   // likely break positive
                                                   // definiteness
    // stick with this simple recipe
    std::cout << "TwoBodyEngine::precision = " << engines[0].precision()
              << std::endl;
    for (size_t i = 1; i != nthreads; ++i) {
        engines[i] = engines[0];
    }
...
Matrix compute_2body_fock_general(const BasisSet& obs, const Matrix& D,
    ...
    std::vector<coulomb_engine_type> engines(nthreads);
    engines[0] =
        coulomb_engine_type(std::max(obs.max_nprim(), D_bs.max_nprim()),
                            std::max(obs.max_l(), D_bs.max_l()), 0);
    engines[0].set_precision(precision);  // shellset-dependent precision
                                          // control will likely break positive
                                          // definiteness
                                          // stick with this simple recipe
    for (size_t i = 1; i != nthreads; ++i) {
        engines[i] = engines[0];
                ...
                for (auto s3 = 0; s3 < D_bs.size(); ++s3) {
                    ...
                    for (auto s4 = s4_begin; s4 != s4_fence; ++s4, ++s1234) {
                        ...
                        if (s3 >= s4) {
                            ...
                            const auto* buf_J = engines[thread_id].compute(
                        ...
                        const auto* buf_K = engines[thread_id].compute(
